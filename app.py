...
# ══════════════════════════════════════════════════════════════════════════════
# GLOBAL PERSISTENT STATE — lives at module level, survives st.rerun()
# The video thread writes here; the Streamlit main thread reads here.
# Never use st.session_state from inside VideoTransformerBase.transform().
# ══════════════════════════════════════════════════════════════════════════════
if "_ps_state" not in st.__dict__:
    st._ps_state = {
        "posture":        "Waiting…",
        "bad_thresh":     0.10,
        "warn_thresh":    0.075,
...
_S = st._ps_state          # short alias — safe across reruns


# ══════════════════════════════════════════════════════════════════════════════
# PAGE CONFIG & CSS
# ══════════════════════════════════════════════════════════════════════════════
st.set_page_config(page_title="PostureSense", page_icon="🧘", layout="wide",
                   initial_sidebar_state="expanded")
...
[data-testid="stSidebar"]{background-color:#0a0c10!important;border-right:1px solid #1e2230}
h1,h2,h3{font-family:'Space Mono',monospace}
...
# SESSION STATE INIT
# ══════════════════════════════════════════════════════════════════════════════
_defaults = dict(session_start=None, session_active=False,
...
# ══════════════════════════════════════════════════════════════════════════════
# SIDEBAR — settings written straight into _S so the video thread sees them
# ══════════════════════════════════════════════════════════════════════════════
with st.sidebar:
    st.markdown("## 🧘 PostureSense")
    st.markdown("<div class='sh'>Detection Settings</div>", unsafe_allow_html=True)

    sensitivity = st.select_slider("Shoulder tilt sensitivity",
                                   options=["Low", "Medium", "High"], value="Medium")
    ...
    for t in ["Keep shoulders level and relaxed.",
              "Ears should align over shoulders.",
              "Sit with hips pushed back in chair.",
              "Screen at eye level, arm's length away.",
              "Take a break every 30 minutes."]:
...
# ══════════════════════════════════════════════════════════════════════════════
# VOICE ALERT — injected once; JS listens for a changing alert-key attribute
# ══════════════════════════════════════════════════════════════════════════════
st.markdown("""
<div id="_ps_alert_trigger" data-key=""></div>
<script>
(function(){
  var lastKey = "";
  var obs = new MutationObserver(function(muts){
    muts.forEach(function(m){
      var el  = document.getElementById("_ps_alert_trigger");
  ...
  var target = document.getElementById("_ps_alert_trigger");
  if(target) obs.observe(target,{attributes:true});
  else document.addEventListener("DOMContentLoaded",function(){
    obs.observe(document.getElementById("_ps_alert_trigger"),{attributes:true});
  });
})();
</script>
""", unsafe_allow_html=True)
...
# ══════════════════════════════════════════════════════════════════════════════
# MEDIAPIPE — IMAGE mode: no timestamp, no freeze
# ══════════════════════════════════════════════════════════════════════════════
@st.cache_resource
def load_detector():
...
# ══════════════════════════════════════════════════════════════════════════════
# VIDEO TRANSFORMER — reads thresholds from _S, writes posture back to _S
# ══════════════════════════════════════════════════════════════════════════════
class PostureDetector(VideoTransformerBase):
    def transform(self, frame):
        img = frame.to_ndarray(format="bgr24")
        ...
        if result.pose_landmarks:
            ...
            # Read live thresholds from shared state
            with _S["lock"]:
                bad_t  = _S["bad_thresh"]
            ...
            cv2.line(img,
                     ...
                     (int(right.x*w), int(right.y*h)), color, 2)
...
# ══════════════════════════════════════════════════════════════════════════════
# READ POSTURE & UPDATE COUNTERS (main thread only)
# ══════════════════════════════════════════════════════════════════════════════
with _S["lock"]:
...
with col_cam:
    ...
    # Voice alert — update the data-key attribute to trigger the MutationObserver
...
with col_stats:
    ...
    if hl:
        ...
        bars = "".join(
            ...
            for v,c in [(
                (1.0,"#00e676") if p=="Good Posture" else
                (0.5,"#ffea00") if p=="Slightly Bent" else
                (0.15,"#ff1744")
            )]
        )
        st.markdown(
    ...
    if not st.session_state.session_active and st.session_state.session_start and total > 0:
        ...
        for ln in [f"**Duration:** {dur//60}m {dur%60}s",
                   ...
                   f"**Alerts triggered:** {st.session_state.alert_count}"]:
            st.markdown(ln)
        if good_pct >= 80:   st.success("Great session! Keep up the excellent posture habits.")
        elif good_pct >= 60: st.warning("Decent session. Try to be more mindful during longer work blocks.")
        else:                st.error("Your posture needs attention. Consider ergonomic adjustments or more frequent breaks.")


# ══════════════════════════════════════════════════════════════════════════════
# AUTO-REFRESH (1 second) while session is active so counters update live
# ══════════════════════════════════════════════════════════════════════════════
if st.session_state.session_active:
    time.sleep(1)
    st.rerun()